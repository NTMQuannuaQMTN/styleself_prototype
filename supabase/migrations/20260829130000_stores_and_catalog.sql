  ...
  create table if not exists public.store_agents (
    ...
    recommendation_limit integer not null default 5
                          check (recommendation_limit between 1 and 8),
    enabled              boolean not null default false,
    updated_at           timestamptz not null default now()
  );

  alter table public.store_agents
    add column if not exists recommendation_limit integer not null default 5;
  ...
  -- ===========================================================================
  -- Public read for a LIVE agent (the embedded /agent/:slug iframe is anonymous)
  -- ===========================================================================
  ...
  create policy stores_select_public on public.stores for select to anon
    using (agent_live = true);

  drop policy if exists store_agents_select_public on public.store_agents;
  create policy store_agents_select_public on public.store_agents for select to anon
    using (exists (
      select 1 from public.stores s where s.id = store_id and s.agent_live
    ));

  drop policy if exists store_locations_select_public on public.store_locations;
  create policy store_locations_select_public on public.store_locations for select to anon
    using (exists (
      select 1 from public.stores s where s.id = store_id and s.agent_live
    ));

  drop policy if exists products_select_public on public.products;
  create policy products_select_public on public.products for select to anon
    using (
      status = 'active'
      and exists (select 1 from public.stores s where s.id = store_id and s.agent_live)
    );

  drop policy if exists variants_select_public on public.product_variants;
  create policy variants_select_public on public.product_variants for select to anon
    using (exists (
      select 1
      from public.products p
      join public.stores s on s.id = p.store_id
      where p.id = product_id and s.agent_live
    ));

  drop policy if exists inventory_select_public on public.inventory;
  create policy inventory_select_public on public.inventory for select to anon
    using (exists (
      select 1
      from public.product_variants v
      join public.products p on p.id = v.product_id
      join public.stores s on s.id = p.store_id
      where v.id = variant_id and s.agent_live
    ));
