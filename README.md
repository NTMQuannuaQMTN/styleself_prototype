...
| `/agent/:agentId?k=<embed_key>` | **Public, no auth** — the deployed Fashion Commerce Agent, embeddable via `<iframe>`. `agentId` = the store's slug; `k` = the store's embed key (backend rejects an anon call whose key doesn't match). `/agent/demo` needs no key. |
...
- **Deploy** — embed snippet (carries a per-store `?k=` embed key; owner can
  regenerate it) + go-live toggle (**owner-only** to publish)
